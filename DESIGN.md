...
- Primary brand color (`{colors.primary}` — #008ae3) and secondary accent (`{colors.secondary}` — #00b0ff) carry every interactive element, paired with navy sidebar/layout colors.
- Two button grammars: tiny blue pill CTAs (`{rounded.pill}`) and compact utility rects (`{rounded.sm}`).
- Plus Jakarta Sans — customized tracking and weighting for the premium and clean One Academy brand feel.
...
- **Primary Navy** (`{colors.primary}` — #0f2854): The main brand-level primary color. All text links, primary buttons, and the focus ring root.
- **Secondary Orange** (`{colors.secondary}` — #ff9d00): A bright secondary accent color used for secondary highlights, loaders, active indicator badges, and accents.
- **Accent Blue** (`{colors.accent-blue}` — #4988c4): Supporting blue accent color.
- **Accent Light** (`{colors.accent-light}` — #bde8f5): Supporting light blue accent color.
- **Accent Cyan** (`{colors.accent-cyan}` — #06b6d4): Supporting cyan accent color.
- **Black** (`{colors.black}` — #101828): Hard dark contrast color.
- **Background** (`{colors.background}` — white): Standard light background canvas.
- **Foreground** (`{colors.foreground}` — #101828): Default text ink color.
- **Border** (`{colors.border}` — #e2e8f0): Border color for cards and partitions.
- **Ring** (`{colors.ring}` — #0f2854): Focus ring color.
- **Canvas** (`{colors.canvas}` — #ffffff): Container card base color.
...
- **Display & Body / UI**: `Plus Jakarta Sans, sans-serif` — The primary font used across the entire platform, configured via `next/font/google` in weights 300 to 800.
- **OpenType features**: OpenType ligatures and kerning are optimized. Display sizes rely on tight tracking.
...
Plus Jakarta Sans is configured directly in the application's root layout (`src/app/layout.tsx`) using `next/font/google`. It is used globally across display, body, and UI elements.
...
**`button-primary`** — The primary action button. Background `{colors.primary}` (LMS Primary Blue #008ae3), text `{colors.on-primary}` in `{typography.body}` (Plus Jakarta Sans 17px / 400), rounded `{rounded.pill}` (full pill — capsule-shaped), padding 11px × 22px. The full-pill radius IS the brand action signal.
...
5. Display headlines stay Plus Jakarta Sans 600. Body stays Plus Jakarta Sans 400 at 17px. The boundary is unbreakable.
